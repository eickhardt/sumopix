 ...
 */
class AzCuttingTool extends Module
{
    /**
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('Cutting Tool');
        $this->description = $this->l('The cutting tool page.');

        $this->confirmUninstall = $this->l('Are you sure you want to uninstall?');

        if (!Configuration::get('AZ_CUTTING_TOOL_NAME'))
    ...
    public function install()
    {
        ...
        if (!parent::install() ||
          ...
          !Configuration::updateValue('AZ_CUTTING_TOOL_NAME', 'Cutting Tool'))
            return false;
        return true;
    }
    ...
    {
        if (!parent::uninstall() || !Configuration::deleteByName('AZ_CUTTING_TOOL_NAME'))
            return false;
        return true;
    }
    ...
    public function hookDisplayLeftColumn($params)
    {
        $this->context->smarty->assign(array(
            'my_module_name' => Configuration::get('AZ_CUTTING_TOOL_NAME'),
            'my_module_link' => $this->context->link->getModuleLink('azcuttingtool', 'display'),
            'my_module_message' => $this->l('This is a simple text message.'),
            'path' => '<span class="navigation_end">'. $this->l('Cutting Tool') .'</span>'
        ));
        return $this->display(__FILE__, 'azcuttingtool.tpl');
    }
    ...
    {
        $this->context->controller->addCSS($this->_path.'css/azcuttingtool.css', 'all');
    }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submit'.$this->name)) {
            $dfm_name = strval(Tools::getValue('AZ_CUTTING_TOOL_NAME'));
            if (!$dfm_name
              || empty($dfm_name)
              || !Validate::isGenericName($dfm_name))
                $output .= $this->displayError($this->l('Invalid Configuration value'));
            else {
                Configuration::updateValue('AZ_CUTTING_TOOL_NAME', $dfm_name);
    ...
    public function displayForm()
    {
        ...
        // Load current value
        $helper->fields_value['AZ_CUTTING_TOOL_NAME'] = Configuration::get('AZ_CUTTING_TOOL_NAME');
